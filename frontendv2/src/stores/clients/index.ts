...
import type { SearchApiResponse, State, SuccessCountResponse, SuccessResponse, ValidationErrorResponse } from './types'
...
export const useClientStore = defineStore('clientStore', {
  ...
  actions: {
    ...
    async getClientById (id:any): Promise<SuccessResponse> {
      const response = await axios.get(`/getClient/${id}`)
      return await response.data

    },
    async addClient (companyDetails: any): Promise<ValidationErrorResponse| SuccessResponse> {
    ...
    async updateClient (companyDetails: any): Promise<ValidationErrorResponse| SuccessResponse> {
      const response  = await axios.put(`updateClient/${companyDetails.id}`, {
        ...companyDetails
      })
      return await response.data
    },
    async deleteClient (clientId: any): Promise<SuccessResponse> {
